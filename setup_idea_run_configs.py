...
DEV_XML = r'''<component name="ProjectRunConfigurationManager">
  <configuration default="false" name="TrainingBackend Dev (MySQL)" type="Application" factoryName="Application">
    ...
    <envs>
      ...
      <env name="DB_NAME" value="training_dev" />
...
(ROOT / ".idea" / "runConfigurations" / "TrainingBackend_Dev_MySQL.xml").write_text(DEV_XML, encoding="utf-8")
(ROOT / ".idea" / "runConfigurations" / "TrainingBackend_Prod_MySQL.xml").write_text(PROD_XML, encoding="utf-8")

# 清理旧的 H2 配置文件
old_file = ROOT / ".idea" / "runConfigurations" / "TrainingBackend_Dev_H2.xml"
if old_file.exists():
    old_file.unlink()

print(f"已生成 IDEA 运行配置到：{TARGET}")
print("请重启 IDEA 或 File → Reload All from Disk，即可在运行配置下拉框中看到：")
print("  1) TrainingBackend Dev (MySQL)   - 开发环境（training_dev 库，DB_PASSWORD 请改为你的密码）")
print("  2) TrainingBackend Prod (MySQL)  - 生产环境（training 库，DB_PASSWORD 请改为你的密码）")
